...
from tools import SirstDataset
...
test_dataset = SirstDataset(mode='val')
train_dataloader = torch.utils.data.dataloader.DataLoader(trainset, batch_size=args.batch_size, shuffle=True)
test_dataloader = torch.utils.data.dataloader.DataLoader(test_dataset, batch_size=args.batch_size)
...
for epoch in range(1, 31):
    ...
    if epoch % 1 == 0: 
        ...
        torch.save(disc_net.state_dict(), "saved_models/discriminator_epoch_%d.pth" % (epoch))
                ...
                # Use the exact test-split names from your dataset
        ...
        # ─── Initialize evaluation sums ──────────────────────────────────
        sum_val_loss_g1         = 0.0
        sum_val_false_ratio_g1  = 0.0
        sum_val_detect_ratio_g1 = 0.0
        sum_val_F1_g1           = 0.0

        sum_val_loss_g2         = 0.0
        sum_val_false_ratio_g2  = 0.0
        sum_val_detect_ratio_g2 = 0.0
        sum_val_F1_g2           = 0.0

        sum_val_loss_g3         = 0.0
        sum_val_false_ratio_g3  = 0.0
        sum_val_detect_ratio_g3 = 0.0
        sum_val_F1_g3           = 0.0
        # ────────────────────────────────────────────────────────────────
        ...
        with torch.no_grad():
            for ix, (x, y) in tqdm(enumerate(test_dataloader), total=len(test_dataloader)):
                ...
                mask = y.to(device, dtype=torch.float)

                # Generator1 output
                g1_result     = gen1_net(img)
                g1_img        = (g1_result.squeeze(0).cpu().numpy() * 255.0)
                g1_img        = np.rollaxis(g1_img, axis=0, start=3)
                cv2.imwrite(f'./training_results/{filenames[ix]}_G1.png', g1_img)

                # Generator2 output
                g2_result     = gen2_net(img)
                g2_img        = (g2_result.squeeze(0).cpu().numpy() * 255.0)
                g2_img        = np.rollaxis(g2_img, axis=0, start=3)
                cv2.imwrite(f'./training_results/{filenames[ix]}_G2.png', g2_img)

                # Fusion output
                fusion_img = ((g1_img + g2_img) / 2).astype(np.uint8)
                cv2.imwrite(f'./training_results/{filenames[ix]}_Res.png', fusion_img)
